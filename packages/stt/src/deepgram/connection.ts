...
export class DeepgramConnection {
  ...
  private async connect(): Promise<void> {
    ...
    try {
      ...
      console.error(`[DG] Failed to create connection for ${this.sessionId}:`, error);
  ...
  private setupEventHandlers(): void {
    ...
    this.connection.on(LiveTranscriptionEvents.Transcript, (result: TranscriptResult) => {
  ...
  async sendAudio(buffer: Buffer, source: AudioSource): Promise<void> {
    ...
    if (!this.isConnected && !this.isConnecting) {
      console.log(`[DG] Lazy connecting for ${this.sessionId} (first audio received)`);
      await this.connect();
  ...
  private async handleTranscript(result: TranscriptResult): Promise<void> {
    ...
    console.log(`[DG] Transcript: "${transcript}" (${is_final ? 'final' : 'partial'})`);
  ...
  private async publishTranscript(result: SttResult): Promise<void> {
    ...
    try {
      ...
      console.error(`[DG] Failed to publish transcript for ${this.sessionId}:`, error);
    }
  }
  ...
  private async reconnect(): Promise<void> {
    ...
    console.log(`[DG] Reconnecting ${this.sessionId} in ${delay}ms (attempt ${this.retryCount})`);
  ...
  async close(): Promise<void> {
    ...
    if (this.connection) {
      try {
        ...
        console.error(`[DG] Error closing connection for ${this.sessionId}:`, error);
      }
